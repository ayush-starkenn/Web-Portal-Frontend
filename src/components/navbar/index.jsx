...
const Navbar = ({ onOpenSidenav }) => {
  ...
  const [newNotificationsFetched, setNewNotificationsFetched] = useState(false);
  ...
  return (
    <>
      ...
      <nav className="sticky top-0 z-10 flex flex-row flex-wrap items-center justify-between bg-white p-2 backdrop-blur-xl dark:!bg-navy-800">
        ...
        <div className="relative mt-[3px] flex h-[35px] w-[355px] flex-grow items-center justify-around gap-2 rounded-full bg-white px-2 py-2  dark:!bg-navy-800 dark:shadow-none md:w-[365px] md:flex-grow-0 md:gap-1 xl:w-[165px] xl:gap-2">
          {isMobile && (
            <span
          ...
          <Dropdown
                ...
                {newNotifications.length > 0 && !read && (
                  <span
                    className={`absolute -right-1 -top-1 h-4 w-4 rounded-full bg-red-500 text-center text-[0.58rem] text-white ${
                      newNotificationsFetched ? "circular-wave-animation " : ""
                    }`}
                    style={{ lineHeight: "2" }}
                  >
            ...
            children={
              <div className="flex w-[360px] flex-col gap-3 rounded-[20px] bg-gray-100 p-4 shadow-xl shadow-shadow-500 dark:!bg-navy-700 dark:text-white dark:shadow-none sm:w-[460px]">
                {newNotifications.length > 0 && read === false ? (
          ...
          >
            {notifications
              ...
              .map((notification, index) => (
                <button className="flex w-full items-center" key={index}>
                  <div className="ml-2 flex h-full w-full cursor-auto flex-col justify-center rounded-lg py-2 pl-8 text-sm">
                    <div key={index}>
                      <p className="my-auto mb-1 flex items-center text-left text-base font-normal text-gray-900 dark:text-white">
                        <IoNotificationsCircle className="mr-2" />
                        {notification.content}
                      </p>
                      <p className="font-base text-left text-xs text-gray-900 dark:text-white">
          ...
          <div
            className={`transform cursor-pointer text-gray-600 transition duration-300 hover:scale-105 `}
            onClick={() => {
