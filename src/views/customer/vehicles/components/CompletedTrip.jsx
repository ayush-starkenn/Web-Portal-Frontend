    ...
    axios
      ...
      .then((res) => {
        ...
        allTripData.forEach((tdata) => {
          if (tdata.spd <= 1 && startHalt === 0) {
            // Start of a potential halt
            startHalt = tdata.timestamp;
          } else if (tdata.spd <= 1) {
            // Continue or update the potential halt
            endHalt = tdata.timestamp;
          } else {
            // Reset if speed is greater than 1
            startHalt = 0;
            endHalt = 0;
          }
          ...
          // Check if the accumulated time is more than 60 seconds
          if (timeDiff > 60) {
            totalHalt = timeDiff;
          }
        });

        // Convert totalHalt to hours, minutes, and seconds
                        ...
                        {marker.reason === 0 ? (
                          <>
                            <div>
                              ...
                              <p className="mb-0">
                                <b>Brake Duration:</b> {marker.brake_duration}{" "}
                                Sec,
                                <b>Bypass:</b>{" "}
                              ...
                              <p className="mb-0">
                                <b>On_X-Axis:</b> {marker.on_x_axis},{" "}
                                <b>Off_X-Axis:</b> {marker.off_x_axis}
                              </p>
                              <p className="mb-0">
                                <b>On_Y-Axis:</b> {marker.on_y_axis},{" "}
                                <b>Off_Y-Axis:</b> {marker.off_y_axis}
                              </p>
                              <p className="mb-0">
                                <b>On_TTC:</b> {marker.on_ttc}, <b>Off_TTC:</b>{" "}
                                {marker.off_ttc}
                              </p>
                              <p className="mb-0">
                                <b>On_Rel-Velocity:</b> {marker.on_rel_velocity}
                                ,<b>Off_Rel-Velocity:</b>{" "}
                                {marker.off_rel_velocity}
                              </p>
                              <p className="mb-0">
                                <b>On_Steering-angle:</b>{" "}
                                {marker.on_steering_angle},
                                <b>Off_Steering-angle:</b>{" "}
